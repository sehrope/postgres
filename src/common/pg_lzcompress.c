 ...
 *			usable only for the last 4095 input positions, since we cannot
 ...
 * 49,152 bytes, a third of the linked list's 147,488.
 * ----------
 */
 ...
 *
 * _pos is the biased position index of _s within the input; the caller
				...
				uint32 pos)
{
	...
	/*
	 * Compute the back-distance to the most recent occurrence of this
	 * hash.  Positions are biased by PGLZ_HISTORY_SIZE (see pglz_compress)
	 * and stored at full width, so an empty (zero) head slot yields a
	 * distance of at least PGLZ_HISTORY_SIZE for any input length and
	 ...
	 * distance is usable only while it fits into a tag (1..0x0ffe); stale
	 * or empty links fall outside that range.
	 */
	while (dist > 0 && dist < 0x0fff)
	{
		const char *ip = input;
		...
		/*
		 * Advance to the previous position with this hash.  The ring slot
		 * for position (pos - dist) still describes that position, since
		 * dist < PGLZ_HISTORY_SIZE.  At full width the stored position is
		 * always strictly older, so the walk cannot loop; the check is
		 * kept as a cheap guard and to stop on a never-written slot.
		 */
		nextdist = (int32) (pos - hist_prev[(pos - dist) & (PGLZ_HISTORY_SIZE - 1)]);
		if (nextdist <= dist)
			break;
		dist = nextdist;
			  ...
			  const PGLZ_Strategy *strategy)
{
	 ...
	 * PGLZ_HISTORY_SIZE instead of 0 makes zero-initialized (empty)
	 * hist_head slots fail pglz_find_match's distance validation.
	 * Unsigned, so incrementing it once per input byte stays defined
	 * for any slen up to INT_MAX.
	 */
			...
			for (ctrlc = 0; ctrlc < 8 && dp < destend; ctrlc++, ctrl >>= 1)
			{
				if (ctrl & 1)
				{
					...
					if (off >= 8 && len <= 16 && destend - dp >= 24)
					{
						 ...
						 * and is overwritten by subsequent output; 16
						 * bytes of slack would suffice for the writes,
						 * 24 simply mirrors the input-side reserve.
