 ...
 *
 *			The compressor creates a table of chains of positions.
 *			For each input position (except the last 3), a hash key is
 *			built from the 4 next input bytes and the position remembered
 *			in the appropriate chain. Thus, the table points to chains
 *			of likely to be at least in the first 4 characters
 *			matching strings. This is done on the fly while the input
 *			is compressed into the output area.  Chain entries remain
 *			usable only for the last 4094 input positions, since we cannot
 *			use back-pointers larger than that anyway; older links simply
 *			fail the distance validation during lookup.  The size of the
 *			hash table is chosen based on the size of the input - a larger
 *			table has a larger startup cost, as it needs to be initialized
 *			to zero, but reduces the number of hash collisions on long
 *			inputs.
 *
 *			For each byte in the input, its hash key (built from this
 *			byte and the next 3) is used to find the appropriate chain
 *			in the table. The chains remember the positions of all bytes
 *			that had the same hash key in the past in increasing backward
 *			offset order. Now for all entries in the used chains, the
 ...
 *
 *			For each subsequent entry in the history chain, the "good_match"
 ...
 *				- there are no more history links to look at
 *				- the next history link is already too far back
 ...
 *			is omitted and only its history link added.
 *
 *		Acknowledgments:
 *
 ...
 * ----------
 */
#define PGLZ_MAX_HISTORY_BUCKETS	8192	/* must be power of 2 */
#define PGLZ_HISTORY_SIZE		4096
#define PGLZ_MAX_MATCH			273
#define PGLZ_MAX_OFF			0x0fff	/* encoder's exclusive max offset;
										 * kept strict for byte-compatibility,
										 * see pglz_find_match */

/* Odd multiplier for Fibonacci hashing, 0x9E3779B1 ~ 2^32 / golden ratio */
#define PGLZ_HASH_MULTIPLIER	2654435761u

/*
 * The ring indexing in pglz_hist_add/pglz_find_match and the empty-slot
 * bias both depend on these relations; see the history array comments.
 */
 ...
 *
 * The history is kept in two position-indexed arrays rather than in
 * linked lists of entry structs:
 *
 * hist_head[h] holds the biased input position (see pglz_compress for the
 * bias) of the most recent occurrence of hash h.  hist_prev[p &
 * (PGLZ_HISTORY_SIZE - 1)] holds, for the input position p stored in that
 * ring slot, the previous position with the same hash.  Walking a chain
 * therefore means computing successive back-distances from the current
 * position; a link is followed only while its distance lies in the usable
 * tag offset range (see pglz_find_match).  Stale, overwritten and
 * never-written entries simply fail that check, so entries never need to
 * be delinked or recycled.
 *
 * Positions are stored at full width rather than truncated to 16 bits.
 * The truncated form would save 24576 bytes of static work array, but
 * cannot distinguish a link to a position more than 65536 back from a
 * nearby one: the subtraction wraps into a small, in-window distance
 * that passes validation, so a chain walk follows it into unrelated
 * data instead of stopping, walking far longer chains than the linked
 * list would.
 *
 * At full width an empty (zero) slot yields a distance of at least
 * PGLZ_HISTORY_SIZE for every input length, so it fails the range check
 * exactly like the linked list's invalid entry did, and chains
 * terminate where the linked list's did.  The static work arrays total
 * 49,152 bytes, a third of the linked list's 147,488 on 64-bit
 * builds.
 * ----------
 */
static uint32 hist_head[PGLZ_MAX_HISTORY_BUCKETS];
 ...
 * hash chain.  This seems an acceptable tradeoff for spreading out the
...
pglz_hist_idx(const char *s, const char *e, int hshift)
{
	...
	v = pg_bswap32(v);
#endif
	return (int) ((v * PGLZ_HASH_MULTIPLIER) >> hshift);
}
 ...
 *
 * pos is the biased position index of s within the input; the caller
 * must increment it by one for every byte added.  The previous head of
 * this hash's chain is remembered in the ring slot for this position,
 * then the head is set to this position.  No delinking is ever needed;
 * stale links fail the distance validation in pglz_find_match.
 * ----------
 */
static inline void
 ...
 * pglz_match_length -
 *
 *		Extend a match between ip and hp, whose first *len* bytes are
 *		already known to be equal, until the first mismatch, *end*, or
 *		PGLZ_MAX_MATCH, whichever comes first.  Returns the total match
 *		length.
 *
 * The bulk of the comparison is done 8 bytes at a time: XOR of two
 * 8-byte loads is zero iff all 8 bytes are equal, and if not, the
 * lowest set bit (little-endian; highest on big-endian) pinpoints the
 * first differing byte.  Loads go through memcpy to avoid alignment
 * assumptions; compilers turn them into single 8-byte loads.
 *
 * hp always trails ip (it points at already-processed input), so both
 * loads are bounded by ip + 8 <= end and cannot read past the input
 * buffer.  A byte-wise loop handles the tail near *end* or near the
 * length cap.
 * ----------
 */
static inline int32
				...
				uint32 pos)
{
	...
	/*
	 * Compute the back-distance to the most recent occurrence of this hash.
	 * Positions are biased by PGLZ_HISTORY_SIZE (see pglz_compress) and
	 * stored at full width, so an empty (zero) head slot yields a distance of
	 * at least PGLZ_HISTORY_SIZE for any input length and fails the range
	 * check below, exactly like the old list's invalid entry did.
	 */
	dist = pos - hist_head[pglz_hist_idx(input, end, hshift)];

	/*
	 * Traverse the history chain until a good enough match is found.  A
	 * distance is usable only while it is below PGLZ_MAX_OFF; stale or empty
	 * links fall outside that range.
	 */
	while (dist > 0 && dist < PGLZ_MAX_OFF)
	{
		 ...
		 * is equal for the same size before computing the exact length.
		 */
		thislen = 0;
		if (len >= 16)
		{
		...
		/*
		 * Advance to the previous position with this hash.  The ring slot for
		 * position (pos - dist) still describes that position, since dist <
		 * PGLZ_HISTORY_SIZE.  At full width the stored position is always
		 * strictly older, so in correct operation the walk cannot loop and
		 * this check is dead; it is a cheap belt-and-suspenders guard against
		 * a corrupted array.
		 */
		 ...
		 * no point in doing calculation if we're at end of the chain.
		 */
			  ...
			  const PGLZ_Strategy *strategy)
{
	...
	/*
	 * Biased input position of dp for the history table.  Starting at
	 * PGLZ_HISTORY_SIZE instead of 0 makes zero-initialized (empty) hist_head
	 * slots fail pglz_find_match's distance validation. Unsigned, so
	 * incrementing it once per input byte stays defined for any slen up to
	 * INT_MAX.
	 */
	uint32		pos = PGLZ_HISTORY_SIZE;
	 ...
	 * hist_prev[] ring; its slots are written before they can be read,
	 * because a slot is only followed from a live chain link.
	 */
