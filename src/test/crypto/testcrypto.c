...
usage(const char *progname)
{
	...
	printf("\n");
	printf(_("  Algorithms supported are AES-GCM and AES-KW.\n"));
	printf("\n");
	printf(_("\nOptions:\n"));
...
main(int argc, char *argv[])
{
	...
	if (algorithm)
	{
		if (strcmp(algorithm,"AES-GCM") == 0)
			cipher = PG_CIPHER_AES_GCM;
		else if (strcmp(algorithm,"AES-KW") == 0)
			cipher = PG_CIPHER_AES_KW;
	...
	else
	{
		...
		key_len = hex_dec_len(key_hex, key_hex_len);
	...
	else if (cipher == PG_CIPHER_AES_GCM)
	{
		...
		iv_len = hex_dec_len(iv_hex, iv_hex_len);
	...
	if (plaintext_hex)
	{
		...
		plaintext_len = hex_dec_len(plaintext_hex, plaintext_hex_len);
	...
	if (ciphertext_hex)
	{
		...
		ciphertext_len = hex_dec_len(ciphertext_hex, ciphertext_hex_len);

		ciphertext = pg_malloc0(ciphertext_len);
		hex_decode(ciphertext_hex, ciphertext_hex_len, (char *) ciphertext);
	}
	...
	if (tag_hex)
	{
		...
		tag_len = hex_dec_len(tag_hex, tag_hex_len);
	 ...
	 * Encryption
	 *
	 * We run through the encryption even if there wasn't a plaintext provided- in
	 * that case we just encrypt the empty string.
	 */
	...
	else if (cipher == PG_CIPHER_AES_KW || cipher == PG_CIPHER_AES_KWP)
	{
	...
	{
		uint64 result_hex_len = hex_enc_len((char *) result, result_len);
		...
		if (cipher == PG_CIPHER_AES_GCM)
		{
			result_hex_len = hex_enc_len((char *) tag_result, tag_len);
	 ...
	 * routine to make sure that we get the correct result, and then error out.
	 */
	 ...
	 * Decryption
	 *
	 * We run through the decryption even if there wasn't a ciphertext provided- in
	 * that case we just decrypt the empty string.
	 */
