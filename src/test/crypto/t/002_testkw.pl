	...
	while (my $line = <$in_txtfile>) {
		...
		if ($line =~ /^[[:space:]]*$/) {
			if (%testrun) {
				...
				if ($testrun{fail}) {
					command_exit_is(\@testargs, 1, "Run $testrun{COUNT} of Plaintext Length: $lengths{'PLAINTEXT LENGTH'}");
				}
				else {
					command_ok(\@testargs, "Run $testrun{COUNT} of Plaintext Length: $lengths{'PLAINTEXT LENGTH'}");
				}
				undef(%testrun);
			}
			else {
