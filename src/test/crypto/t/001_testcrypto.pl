...
my $algo = "AES-GCM";
# my @rspfiles = ("gcmDecrypt128.rsp", "gcmDecrypt192.rsp", "gcmDecrypt256.rsp", "gcmEncryptExtIV128.rsp", "gcmEncryptExtIV192.rsp", "gcmEncryptExtIV256.rsp");
my @rspfiles = ("gcmDecrypt128.rsp", "gcmDecrypt256.rsp", "gcmEncryptExtIV128.rsp", "gcmEncryptExtIV256.rsp");
	...
	while (my $line = <$in_rspfile>) {
		...
		if ($line =~ /^[[:space:]]*$/) {
			if (%testrun) {
				...
				if ($testrun{fail}) {
					command_exit_is(\@testargs, 1, "Run $testrun{Count} of Keylen: $lengths{Keylen}, IVlen: $lengths{IVlen}, PTlen: $lengths{PTlen}, AADlen: $lengths{AADlen}, Taglen: $lengths{Taglen}");
				}
				else {
					command_ok(\@testargs, "Run $testrun{Count} of Keylen: $lengths{Keylen}, IVlen: $lengths{IVlen}, PTlen: $lengths{PTlen}, AADlen: $lengths{AADlen}, Taglen: $lengths{Taglen}");
				}
				undef(%testrun);
			}
			else {
