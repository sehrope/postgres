...
# All records the --relation, --block and --fork filter tests can match
# lie before this point: the TRUNCATE above gave t1 and i1a new
# relfilenodes, so later records never match their table oids, and t2
# and i2 are the only objects with init forks.  Those tests stop
# reading here.
my $rel_test_end_lsn =
...
# End of the deliberately generated records that the option and filter
# tests below look at.  The WAL emitted after this point only fills up
# the segment to set up the continuation record case, so those tests
# stop here rather than decode all of it every time.
my $test_end_lsn =
  $node->safe_psql('postgres', 'SELECT pg_current_wal_insert_lsn()');
...
# Decode the full WAL range and check the output format once.  Rendering
# does not depend on how the input is read, so there is no need to
# repeat this multi-megabyte dump for every scenario below.
{
	my @lines = test_pg_waldump($node->data_dir, $start_lsn, $end_lsn);
	is(grep(!/^rmgr: \w/, @lines), 0, 'all output lines are rmgr lines');
}

# The filter tests also run once, against the plain WAL directory, since
# filtering is independent of how the input is read as well.  Repeating
# them per scenario would decode the WAL dozens more times, which is
# what made this test so slow on Windows.  They run over all of the
# deliberately generated records, stopping at test_end_lsn before the
# segment-filling workload, which has nothing more to offer them.
{
	...
	# These three filters can only match records written before
	# rel_test_end_lsn, so there is no point reading further.
  ...
  SKIP:
	{
		...
		# The whole generated WAL range parses without error from each
		# archive.  Using --quiet keeps the sizable record dump off
		# stdout while every record is still decoded.  The plain
		# directory needs no equivalent, since the full range was
		# already decoded from it above.
		if ($scenario->{'is_archive'})
		{
			command_like(
				[
		...
		# The following tests only need the records near the end of the
		# generated WAL.  Starting at contrecord_lsn instead of start_lsn
		# avoids decoding the large segment-filling workload every time.
		command_fails_like(
			[
				'pg_waldump',
				'--path' => $path,
