...
# segment-filling workload, which has nothing more to offer them.
{
	...
	@lines =
	  test_pg_waldump($path, $start_lsn, $test_end_lsn, '--fork' => 'init');
  ...
  SKIP:
	{
		...
		# The whole generated WAL range parses without error.  Using
		# --quiet keeps the sizable record dump off stdout while every
